...
if not results:
    # JIKA KOSONG: Suntikkan baris "CASH IS KING" agar aplikasi tidak terlihat mati
    ...
    results.append({
        ...
        "power_asing": 0.0,
        "modal_asing": 0,
        "status": "🛡️ CASH IS KING",
        "katalis": "Market rawan rontok! Tidak ada saham yang lolos filter ketat AI hari ini."
    })
    print("⚠️ Tidak ada saham lolos. Mode Proteksi Modal (CASH IS KING) dikirim ke Database.")

try:
    # Kita TETAP hapus data kemarin setiap hari, apapun yang terjadi
...
# Masukkan hasil (Entah itu saham beneran, atau sekadar sinyal CASH)
supabase.table('jii30_daily_data').insert(results).execute()
print(f"[{datetime.now(timezone.utc)}] 🎉 Sukses menyimpan {len(results)} baris ke Database!")